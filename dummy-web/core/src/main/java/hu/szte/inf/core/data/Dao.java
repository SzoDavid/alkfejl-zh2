...
public interface Dao<ID, T> {
     ...
     * If model is saved without an ID, and the save was successful,
     * the ID field will be populated.
     *
     ...
     * <p/>
     * To make a list from an iterable use:
     * {@code StreamSupport.stream(findAll().spliterator(), false).toList()},
     * however usual implementations return an arraylist anyway,
     * so you could simply cast as {@code (ArrayList<T>) findAll()}
     */
    Iterable<T> findAll();
